...
## read data
data <- read.table('./table/lulc_reference_cerrado.csv', 
                   header= TRUE,
                   sep= ',')
...
## retain only native
data <- subset(data, class_str == 'Formação Florestal' |
                 class_str == 'Formação Savânica' |
                 class_str == 'Áreas Úmidas' |
                 class_str == 'Formação Campestre')
...
## agregate biome data
data_sum_ref <- aggregate(x=list(area= data$area), by= 
                      list(class_str= data$class_str),
                      FUN= 'sum')

## plot
ggplot(data= data_sum_ref, mapping= aes(x= class_str, y= area/1e6)) +
