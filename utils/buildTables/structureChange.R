...
#######
## read edge area data
data <- read.table('../table/structural_change.csv', header= TRUE, sep= ',')
...
x <- aggregate(x=list(area= data$area), by= list(value= data$value, ecoregion= data$ecoregion, variable= data$variable), FUN='sum')
...
## export table 
write.table(x= subset(recipe, value != 0),
            file= '../data_studio/structuralChange_v3.csv', 
            fileEncoding='UTF-8',
            row.names= FALSE,
            sep='\t',
            dec=',',
